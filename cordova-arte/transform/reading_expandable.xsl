...
<!--This file transforms artetranscription_clean XML into a reading copy of the Arte transcription
    ...
    on the right.-->
<!-- For transformation scenario, the output should be reading_expandable_text.html in the _includes directory within jekyll -->
...
<xsl:template mode="content" match="div">
    ...
    <xsl:template mode="outline" match="div">
        <li data-id="{@xml:id}"><xsl:value-of select="head[@type='outline']"/>
